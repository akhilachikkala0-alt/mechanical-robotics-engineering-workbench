...
// Persistent SQLite database file matching robotic_arm_dms schema
const DB_FILE = path.resolve(process.cwd(), 'robotic_arm_dms.db');
export const db = new DatabaseSync(DB_FILE);

// Enable foreign keys
db.exec('PRAGMA foreign_keys = ON;');

// Live SQL Query Logger for academic and demonstration transparency
...
const MAX_LOGS = 100;
export const sqlQueryLogs: SqlLogEntry[] = [];

export function logQuery(sql: string, params: any[] = [], durationMs: number = 0, rowCount: number = 0) {
  ...
  if (sqlQueryLogs.length > MAX_LOGS) {
    sqlQueryLogs.pop();
  }
}
...
export function queryAll<T = any>(sql: string, params: any[] = []): T[] {
  ...
  try {
    ...
    logQuery(sql, params, duration, rows.length);
    return rows;
  } catch (err: any) {
    logQuery(`[ERROR] ${sql}`, params, performance.now() - start, 0);
    throw err;
  }
}
...
export function queryOne<T = any>(sql: string, params: any[] = []): T | undefined {
  ...
  try {
    ...
    const duration = performance.now() - start;
    logQuery(sql, params, duration, row ? 1 : 0);
    return row;
  } catch (err: any) {
    logQuery(`[ERROR] ${sql}`, params, performance.now() - start, 0);
    throw err;
...
export function execute(sql: string, params: any[] = []): { changes: number; lastInsertRowid: number | bigint } {
  ...
  try {
    ...
    const duration = performance.now() - start;
    logQuery(sql, params, duration, Number(result.changes));
...
export function initDatabase() {
  console.log('[DBMS] Initializing database: robotic_arm_dms...');
  ...
  db.exec(`
    CREATE TABLE IF NOT EXISTS Engineer (
      ...
      role TEXT DEFAULT 'Robotics Design Engineer',
      department TEXT DEFAULT 'Mechanical & Mechatronics R&D',
      phone TEXT,
    ...
    CREATE TABLE IF NOT EXISTS Bearing (
      ...
      width REAL NOT NULL,
      dynamic_load_rating REAL NOT NULL,
      static_load_rating REAL NOT NULL,
    ...
    CREATE TABLE IF NOT EXISTS Material (
      ...
      corrosion_resistance TEXT DEFAULT 'Good',
      machinability TEXT DEFAULT 'High',
      FOREIGN KEY (component_id) REFERENCES Component(component_id) ON DELETE CASCADE
    );

    CREATE TABLE IF NOT EXISTS Gripper (
      gripper_id INTEGER PRIMARY KEY AUTOINCREMENT,
      component_id INTEGER NOT NULL UNIQUE,
      gripper_type TEXT NOT NULL,
      max_payload REAL NOT NULL,
      stroke REAL NOT NULL,
      grip_force REAL NOT NULL,
      actuation_type TEXT DEFAULT 'Electric / Servo',
    ...
    CREATE TABLE IF NOT EXISTS Component_Supplier (
    ...
    CREATE TABLE IF NOT EXISTS Robot_Component (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      robot_id INTEGER NOT NULL,
      component_id INTEGER NOT NULL,
      quantity INTEGER NOT NULL DEFAULT 1,
      role_or_joint TEXT DEFAULT 'Joint / Structure',
  ...
  // Seed sample data if Engineer table is empty
  const countRow = db.prepare('SELECT COUNT(*) as count FROM Engineer').get() as { count: number };
  if (countRow.count === 0) {
    seedDatabase();
  }
}
...
function seedDatabase() {
  console.log('[DBMS] Seeding sample data into robotic_arm_dms...');
  ...
  db.exec(`
    ...
    (2, 10, 'Carbon Fiber Composite', 1.55, 850.00, 1500.00, 135.00, 'Excellent', 'Moderate'),
    ...
    -- PickBot-01 BOM (Robot_Component)
    -- Total: 6x9000 + 4x850 + 3x3200 + 1x6500 + 3x1200 + 1x650 = 77,150
  ...
  `);
  console.log('[DBMS] Sample data successfully inserted into robotic_arm_dms.');
}
